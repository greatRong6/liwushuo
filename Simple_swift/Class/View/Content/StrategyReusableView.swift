...
class StrategyReusableView: UICollectionReusableView {
    ...
    override init(frame: CGRect) {
        ...
        self.titleLabel = UILabel(frame: CGRect(x: frame.size.width/2 - 25,y: 5, width:50,height: 40))
        ...
        let line = UIView.init(frame: CGRect(x: 10,y: 25,width: frame.size.width/2 - 45,height: 1))
        line.backgroundColor = RGB(r: 230, g: 230, b: 230)
        self.addSubview(line)
        
        let line1 = UIView.init(frame: CGRect(x: frame.size.width/2 + 35,y: 25,width: frame.size.width/2 - 45,height: 1))
