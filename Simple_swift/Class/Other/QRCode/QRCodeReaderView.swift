...
class QRCodeReaderView: UIView, AVCaptureMetadataOutputObjectsDelegate{
    ...
    override init(frame: CGRect) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(creatDrawLine), name: NSNotification.Name(rawValue: "LAYERANIMATION"), object: nil)
    ...
    override func draw(_ rect: CGRect) {
        ...
        let y = CGFloat(100)//kScreenHeight / 2 - scanZoneSize.height / 2 - CGFloat(KNavigaHeight)
    ...
    func creatDataSource() {
        ...
        if device!.isFocusPointOfInterestSupported && device!.isFocusModeSupported(AVCaptureDevice.FocusMode.continuousAutoFocus) {
            do {
                ...
                self.input?.device.focusMode = AVCaptureDevice.FocusMode.continuousAutoFocus
...
//MARK: ---扫描线
    @objc func creatDrawLine() {
        //(kScreenHeight - (kScreenWidth - 60 * 2)) / 2
        let rect = CGRect(x: 60 + 10, y: 100, width: kScreenWidth - 60 * 2 - 20, height: 2)
        if self.lineImageView == nil {
        self.lineImageView = UIImageView.init(frame: rect)
        self.lineImageView?.image = UIImage(named: "QRCode_ScanLine")
        self.addSubview(self.lineImageView!)
        }
        
        let transitionAnimation = CABasicAnimation.init(keyPath: "position")
        transitionAnimation.fromValue = NSValue.init(cgPoint: CGPoint(x: 60 + (kScreenWidth - 60 * 2) / 2, y: 100))
        transitionAnimation.toValue = NSValue.init(cgPoint: CGPoint( x: 60 + (kScreenWidth - 60 * 2) / 2, y: kScreenWidth - self.defaultBothSideWidth * 2 + 100))
...
//MARK: ---继续动画的方法
    ...
    func creatOtherView(_ scanRect : CGRect) {
        ...
        bottomView = UIView.init(frame: CGRect(x: 0,y: scanRect.maxY, width: kScreenWidth, height: kScreenHeight - scanRect.maxY - 100 - CGFloat(KNavigaHeight)))
        ...
        mineBtn.addTarget(self, action: #selector(pushInMineQRCode), for: .touchUpInside)
...
//MARK: ---设置底部的按钮
    func setBottomBtnView() {
        ...
        btnBottomView = UIView.init(frame: CGRect(x: 0, y: kScreenHeight - btnBottomViewHeight - CGFloat(KNavigaHeight), width: kScreenWidth, height: btnBottomViewHeight))
        ...
        leftQRCodeBtn.addTarget(self, action: #selector(scanQRCode(_:)), for: .touchUpInside)
        ...
        lightBtn.addTarget(self, action: #selector(turnOnOrOffWigthLight(_:)), for: .touchUpInside)
        ...
        barCodeBtn.addTarget(self, action: #selector(scanBarCode(_:)), for: .touchUpInside)
        barCodeBtn.titleLabel?.font = UIFont.systemFont(ofSize: 14)
        btnBottomView.addSubview(barCodeBtn)
    }
    
//MARK: ---点击开灯按钮的事件
    @objc func turnOnOrOffWigthLight(_ sender : UIButton) {
    ...
    func theLightIsON(_ turnLight : Bool){
        if device != nil && device!.hasTorch {
            do{
                ...
                input?.device.torchMode = turnLight ? AVCaptureDevice.TorchMode.on : AVCaptureDevice.TorchMode.off
...
//MARK: ---扫描二维码
    @objc func scanQRCode(_ sender : UIButton) {
...
//MARK: ---扫描条形码
    @objc func scanBarCode(_ sender : UIButton) {
        if sender.isSelected {
            return
        }
...
//MARK: ---创建扫描区域
    func creatScanZone(_ rect: CGRect) -> CGRect {
        ...
        print("x:--\(x) y:--() width:-- %f\(height)  height: -- %f\(width)")
